...
- **自定义主题**：另开一页调五个颜色 + 卡片透明度 + 背景图（取景 / 缩放 / 模糊 / 淡化），整页实时预览，颜色从底色现推
- **毛玻璃卡片**：卡片带 `backdrop-filter`，自定义主题下调低卡片透明度时，背景图会糊开再透上来
...
34. **自定义主题是「几个颜色进、整套变量出」，不存调色板** — 第 25 条把「加主题」的成本压到「加一份变量表」，但那是**开发者**加；让**用户**自己配就不能这么算了：用户改的是背景 / 卡片 / 主色 / 正文 / 次要文字这五个颜色，存储里也只留这五个（外加卡片透明度和背景图那几项），其余三十来个变量由 `buildCustom()` 现推。存整套变量的话，以后往调色板里补一个变量，所有老用户手里那份旧表都会缺那一项 —— 正是第 25 条那个「必须逐字对应」的纪律在用户数据上的翻版，而且这次没人能靠改代码修好它。
    ...
    - **编辑页单独开一页，因为可调项从 3 个涨到十几个**（五个颜色 + 卡片透明度 + 背景图那五项），铺在设置页里会把「外观」那一栏撑得比别的栏长出一大截；而**这一页本身就是预览**，比一个小色块直观得多。三条约定：进这一页即写 `theme: 'custom'`（否则会出现「颜色改了、整机还是深色」）；滑块 `changing` 只改界面、`change` 才落盘（拖一次几十个事件，每次同步写存储没有必要）；这一页**不参与页面淡入** —— 页里有 `slider` / `image` 这些原生组件，它们不认祖先的 `opacity`，淡入时会看到「底片全透明、滑块已经杵在那儿」。
...
35. **背景图存本地文件，storage 里只留一个路径 —— 于是「谁负责删」成了必须回答的问题** — 那是张相册图，压过一道也常有几百 KB，而 storage 一共只有 10MB（见数据模型），所以走 `saveFile` 存进本地文件，storage 里只留路径。由此推出的五件事：
    ...
    - **取景（左右 / 上下 / 缩放）只能靠「整张没裁过的图 + 外层裁切」，不能用 `background-image`。** 想要「横屏图自己挑露哪一块」，先得让那张图完整地装进元素里，再由外面那层 `overflow: hidden` 裁出可见的一窗 —— 元素要比屏幕大，所以必须知道**原图宽高比**（`imageW/H`，选图时用 `getImageInfo` 问出来）才能算出「铺满」要多大。三条路都试过：`background-image` + `background-size: cover` + `background-position: %` 看着最贴切，**但小程序不支持** —— `wxfile://` 这类本地路径在 WXSS 的 `url()` 里解析不了（编译期解析，真机上是一片空白），所以哪怕写法最对也不能用；`mode="aspectFill"` 永远居中裁，给不了取景的余地。
...
36. **卡片加毛玻璃，弹层不加** — `backdrop-filter: blur(16px) saturate(140%)` 写在 `.card` 上。它**只在卡片真的半透明时才看得出来**：深 / 浅色两套的 `surfaceAlpha` 恒为 100%，卡片本就是实心，糊的是它自己背后那层已经被它盖住的底色 —— 所以那两套观感逐像素不变，这是刻意的（见第 34 条同一句话）。真正看得见的是自定义主题 + 背景图那条路：卡片半透明，背后的图被糊开、又透上来一点，比直接透一张全清晰的图干净得多。老基础库不认这个属性会整条丢掉，退化成原来的半透明卡片，不会坏。
...
| 卡片毛玻璃 | 只在卡片真的半透明时看得见（自定义主题 + 背景图）；深 / 浅色两套 `surfaceAlpha` 恒为 100%，观感不变。弹层不加：它底下是黑遮罩，糊了也看不见（见决策 36） |
| 背景图占空间 | 存本地文件而非 storage，storage 只留路径；换图 / 移除 / 恢复默认 / 清空数据 / 导入覆盖都会删掉旧文件（见决策 35） |
| 横屏背景图 | 取景三项（左右 / 上下 / 缩放）由 `theme.bgLayer()` 统一算几何，编辑页预览与五个页面共用同一个函数（见决策 35） |
| 背景图问不到尺寸 | 退回 `aspectFill` 铺满居中，也就是这一版之前的表现；拿不到宽高比时任何取景计算都是猜（见决策 35） |
...
- **Custom theme**: a dedicated page for five colors plus card alpha and a background image (framing / zoom / blur / dim); the page is the live preview, and the rest of the palette is derived from the background color
- **Frosted-glass cards**: `backdrop-filter` on every card, so lowering card alpha under a custom theme blurs the background image before letting it through
...
| Frosted cards | Visible only where cards are genuinely translucent (custom theme + background image); `surfaceAlpha` is always 100% in the dark and light palettes, so those are unchanged. Sheets are excluded — they sit on a black scrim, so there is nothing to see through (see #36) |
...
| A landscape background image | The three framing controls (left-right / up-down / zoom) are computed by one function, `theme.bgLayer()`, shared by the editor preview and all five pages (see #35) |
| Background image with unknown dimensions | Falls back to `aspectFill`, filled and centred — how it behaved before this version; without an aspect ratio any framing computation is a guess (see #35) |
